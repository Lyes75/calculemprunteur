...
interface InsurerFAQProps {
  insurer: InsurerData;
}

export default function InsurerFAQ({ insurer }: InsurerFAQProps) {
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: insurer.faq.map((item) => ({
      "@type": "Question",
      name: item.question,
      acceptedAnswer: {
        "@type": "Answer",
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900">
            Questions fr&eacute;quentes : {insurer.name} assurance emprunteur
          </h2>
        </div>

        <div className="space-y-3">
          {insurer.faq.map((item, index) => (
            <details
              key={index}
